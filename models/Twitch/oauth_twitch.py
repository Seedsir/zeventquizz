...
class TwitchAuth:
    ...
    def get_token(code):
        ...
        user = twitch.create_user(twitch)
        return redirect(f'http://localhost:3000/redirect/'
                        f'{twitch.access_token}/'
                        f'{user["username"]}')
    ...
    def create_user(twitch: 'TwitchAuth') -> User:
        ...
        if not user.is_user_already_exist(user_id):
            ...
            user.create_user(username, user_id, twitch.access_token, twitch.refresh_token)
            return user.render()
        user = user.get_user_by_id_twitch(user_id)
        logger.info(
            f"L'utilisateur existe et les données sont récupéré en db. Type de user: {type(user)} et sa valeur: {user}")
        user.refresh_token = twitch.refresh_token
        user.access_token = twitch.access_token
